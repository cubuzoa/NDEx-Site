/******************************************************************************
 * Creates a database and populates it with test data. Requires a clean copy
 * of the database.
 ******************************************************************************/
...
function processUserFile(filename)
{
    ...
    ndex.createUser(username, password, email,
        function(userData)
        {  // success handler for create user
...
var testUserFiles = getTestUserFileNames();

for (i in testUserFiles)
{
    if (testUserFiles[i].substring(0, 1) != ".")
        processUserFile(testUserFiles[i]);
}
