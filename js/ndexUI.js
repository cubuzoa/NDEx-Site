...
function isOwnedNetwork(jid) {
    if (user && user.ownedNetworks && user.ownedNetworks.length > 0) {
        $.each(user.ownedNetworks, function (index, network) {
            if (network.jid == jid) return true;
        });
...
function isOwnedGroup(jid) {
    if (user && user.ownedGroups && user.ownedGroups.length > 0) {
        $.each(user.ownedGroups, function (index, group) {
            if (group.jid == jid) return true;
        });
    }
    return false;
}
